...
- The set V is partitioned into two different types of nodes:
  ...
  - P = {P~1~, .. P~n~ } ie the set of active processes.

  - R = {R~1~, .. R~m~ } ie the set of resource types.

- Edges are of two types

  - **Request edge**

    P~i~ => R~j~ signifies that process P~i~ has requested an instance of resource R~j~, and is currently waiting for that resource.

  - **Assignment edge**

    R~j~ => P~i~ signifies that an instance of resource type R~j~ has been allocated to process P~i~.
...
- £ = { P~1~=>R~1~, P~2~=>R~3~, R~1~=>P~2~, R~2~=>P~2~, R~2~=>P~1~, R~3~=>P~3~ }
...
4. Circular wait
   ...
   - P~1~ if requests R~2~, cannot request for R~1~. It can only request for resources greater than R~2~ like to R~3~, R~4~, etc.
...
- A system is in a safe state only if there exists a safe sequence. A sequence of processes < P~1~, P~2~, P~n~ > is a safe sequence for the current allocation state if, for each process, the resource that it requests can still be satisfied by the currently available resources plus the resources held by all processes.
...
At time t~0~, the system is in a safe state. The sequence < P~1~, P~0~, P~2~ > satisfies the safety condition.

A system can go from a safe state to an unsafe state.

Suppose at time t~1~, process P~2~ requests and is allocated one more tape drive. Then the system is no longer in a safe state.
...
A claim edge P~i~ -> R~j~ indicates that process P~i~ may request resource R~j~ at some time in the future.

![](/images/2022-02-07-20-40-10-image.png)

When P~1~ requests R~2~, the claim edge P~1~->R~2~ is converted to a request edge. Similarly, when R~2~ is releases by P~1~, the assignment edge R~2~->P~1~ is reconverted to a claim edge P~1~->R~2~

The resources must be claimed a priori in the system. Ie before process P~i~ starts executing, all its claim edges must already appear in the resource allocation graph.

Suppose that P~i~ requests R~j~. The request can be granted only if converting the request edge P~i~->R~j~ to and assignment edge R~j~->P~i~ does not result in the formation of a cycle in the resource allocation graph.
...
1. Available:
   ...
   - `available[j] = k;` means that there are `k` instances of resource type R~j~.

2. Max:

   - 2D array of size `n x m`, that specifies the maximum demand of each process in a system. `int max[n][m];`

   - `max[i][j] = k` means that P~i~ may request `k` instances of resource type R~j~.
...
3. Allocation:
   ...
   - `allocation[i][j] = k` means that P~i~ is currently allocated `k` instances of resource type R~j~.

4. Need:

   - 2D array of size `n x m`, that indicates the remaining resource needs of each process. `int need[n][m];`

   - `need[i][j] = k` means that P~i~ currently needs `k` instances of resource type R~j~ for its execution.
...
2. If request~i~ <= available, go to step 3
   ...
   Otherwise P~i~ must wait, since the resources are not available.

3. Have the system pretend to have allocated the requested resources to process P~i~ by modifying the states as follows:
...
- An edge from P~i~ to P~j~ indicates that P~i~ is waiting for P~j~ to release a resource that is needed by P~i~.
...
3. Request
   ...
   If `request[i][j] = k`, then P~i~ is requesting `k` more instances of resource type R~j~.
